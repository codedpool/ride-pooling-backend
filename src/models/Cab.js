  ...
  static async findNearby(lat, lon, radiusKm = 5) {
    const query = `
      ...
      WHERE is_active = true
        AND ST_DWithin(current_location::geography, ST_SetSRID(ST_MakePoint($2, $1), 4326)::geography, $3)
      ORDER BY distance_km
    `;
    const result = await db.query(query, [lat, lon, radiusKm * 1000]); // Convert km to meters
    return result.rows;
  }
